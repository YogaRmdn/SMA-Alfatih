...
<x-admin.page-header title="Kelola Halaman" subtitle="Kelola Halaman Konten Statis Website">
    <x-slot:button>
        ...
        <x-admin.delete-all :route="'admin.pages.delete-all'" message="Semua halaman beserta gambarnya akan dihapus secara permanen. Lanjutkan?" />
    </x-slot:button>
</x-admin.page-header>
...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="divide-y divide-slate-100 lg:hidden">
            ...
            <div class="flex items-center gap-3 p-4">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-semibold text-slate-800">
                        <a href="{{ route('admin.pages.edit', $page) }}" class="hover:text-emerald-700">{{ $page->title }}</a>
                    </p>
                    <p class="truncate font-mono text-[12px] text-slate-500">{{ $page->slug }}</p>
                    <div class="mt-1.5 flex flex-wrap items-center gap-1.5">
                        <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-[12px] font-medium text-slate-600">{{ ucfirst($page->section) }}</span>
                        @if ($page->is_active)
                            <span class="rounded-full bg-emerald-100 px-2.5 py-0.5 text-[12px] font-semibold text-emerald-700">Aktif</span>
                        @else
                            <span class="rounded-full bg-amber-100 px-2.5 py-0.5 text-[12px] font-semibold text-amber-700">Nonaktif</span>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="px-5 py-12 text-center text-sm text-slate-400">Belum ada halaman.</div>
        @endforelse
    </div>
