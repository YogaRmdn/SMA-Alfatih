...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 text-left text-xs uppercase tracking-wider text-slate-500">
                    ...
                    <th class="px-5 py-3">Pendidikan</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($staffs as $staff)
                    <tr class="transition hover:bg-slate-50">
                        ...
                        <td class="px-5 py-3 text-slate-600">{{ $staff->position }}</td>
                        <td class="px-5 py-3 text-slate-600">{{ $staff->education ?? '-' }}</td>
                        <td class="px-5 py-3">
                            @if ($staff->is_active)
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-12 text-center text-slate-400">Belum ada data staff.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="border-t border-slate-100 p-4">
        <x-admin.pagination :paginator="$staffs" />
    </div>
</div>
@endsection
